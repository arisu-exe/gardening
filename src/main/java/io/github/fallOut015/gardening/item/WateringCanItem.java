...
public class WateringCanItem extends Item {
    ...
    public ActionResultType useOn(ItemUseContext itemUseContext) {
        ...
        if(level.isClientSide) {
            level.addParticle(ParticleTypes.RAIN, (double) pos.getX() + itemUseContext.getLevel().getRandom().nextDouble(), (double) pos.getY() + (state.getMaterial().isSolid() ? 1.5d : 0.5d), (double) pos.getZ() + itemUseContext.getLevel().getRandom().nextDouble(), 0, 0, 0);
            level.addParticle(ParticleTypes.RAIN, (double) pos.getX() + itemUseContext.getLevel().getRandom().nextDouble(), (double) pos.getY() + (state.getMaterial().isSolid() ? 1.5d : 0.5d), (double) pos.getZ() + itemUseContext.getLevel().getRandom().nextDouble(), 0, 0, 0);
        }
